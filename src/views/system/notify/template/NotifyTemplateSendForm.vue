  ...
  <Dialog v-model="dialogVisible" :title="t('system.notify.template._todo153')" :max-height="500">
    ...
    >
      ...
      <el-form-item
        ...
        :key="param"
        :label="t('system.notify.template.paramLabel', { param })"
        :prop="'templateParams.' + param"
      >
        <el-input
          v-model="formData.templateParams[param]"
          :placeholder="t('system.notify.template.inputParam', { param })"
        />
      </el-form-item>
    </el-form>
    <template #footer>
...
const open = async (id: number) => {
  ...
  try {
    ...
    formRules.templateParams = data.params.reduce((obj, item) => {
      obj[item] = { required: true, message: t('system.notify.template.paramRequired', { param: item }), trigger: 'blur' }
      return obj
    }, {})
  } finally {
...
const submitForm = async () => {
  ...
  try {
    ...
    if (logId) {
      message.success(t('system.notify.template.sendSuccessLog', { logId }))
    }
