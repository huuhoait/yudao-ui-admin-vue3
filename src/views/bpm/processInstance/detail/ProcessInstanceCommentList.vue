<template>
  <div class="min-h-100% py-24px px-28px" v-loading="props.loading || commentLoading">
    ...
    >
      <div class="text-18px font-bold text-[var(--el-text-color-primary)]">{{ t('bpm.processInstance.detail._todo195') }}</div>
      <div class="text-13px text-[var(--el-text-color-secondary)]">共 {{ comments.length }} 条</div>
    </div>
    <el-empty v-if="!comments.length" :description="t('bpm.processInstance.detail._todo194')" />
    <el-timeline
      v-else
...
const getCommentText = (type: string) => {
  return (getDictObj(DICT_TYPE.BPM_COMMENT_TYPE, type)?.label || t('bpm.processInstance.detail._todo196')).substring(0, 1)
}
