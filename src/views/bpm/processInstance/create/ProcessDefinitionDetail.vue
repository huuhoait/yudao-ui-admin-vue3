    ...
    <div class="processInstance-wrap-main">
      <el-scrollbar>
        ...
        <el-tabs v-model="activeTab">
          <!-- 表单信息 -->
          <el-tab-pane :label="t('bpm.processInstance.create.fillForm')" name="form">
          ...
          <!-- 流程图 -->
          <el-tab-pane :label="t('bpm.processInstance.create.processDiagram')" name="diagram" lazy>
...
const getApprovalDetail = async (row: any) => {
  try {
    ...
    if (!data) {
      message.error(t('bpm.processInstance.create.approvalDetailNotFound'))
      return
    }
...
const submitForm = async () => {
  ...
  try {
    ...
    console.warn(t('bpm.processInstance.create.formValidationFailed'), error)
    return
  }
  // 如果有指定审批人，需要校验
  if (startUserSelectTasks.value?.length > 0) {
  ...
  try {
    ...
    // 提示
    message.success(t('bpm.processInstance.create.processStartedSuccessfully'))
    // 跳转回去
