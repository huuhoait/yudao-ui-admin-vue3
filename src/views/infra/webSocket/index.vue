<template>
  <doc-alert :title="t('infra.webSocket._todo367')" url="https://doc.iocoder.cn/websocket/" />
...
watchEffect(() => {
  ...
  try {
    ...
    if (!type) {
      message.error(t('infra.webSocket.unknownMessageType', { data: data.value }))
      return
    }
    ...
    if (type === 'demo-message-receive') {
      ...
      if (single) {
        messageList.value.push({
          text: t('infra.webSocket.singleSendPrefix', { userId: content.fromUserId, text: content.text }),
          time: new Date().getTime()
        })
      } else {
        messageList.value.push({
          text: t('infra.webSocket.broadcastPrefix', { userId: content.fromUserId, text: content.text }),
          time: new Date().getTime()
        })
      }
      return
    ...
    if (type === 'notice-push') {
      messageList.value.push({
        text: t('infra.webSocket.systemNotifyPrefix', { title: content.title }),
        time: new Date().getTime()
      })
      return
    }
    message.error(t('infra.webSocket.unhandledMessage', { data: data.value }))
  } catch (error) {
    message.error(t('infra.webSocket.messageError', { data: data.value }))
    console.error(error)
  }
})
